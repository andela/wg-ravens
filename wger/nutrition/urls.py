...
patterns_plan = [url(r'^overview/$', plan.overview, name='overview'),
                 ...
                 url(r'^(?P<pk>\d+)/delete/$',
                     ...
                     name='delete'), url(r'^(?P<pk>\d+)/edit/$', login_required(
        plan.PlanEditView.as_view()), name='edit'), url(
        r'^(?P<id>\d+)/pdf/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})$',
        plan.export_pdf, name='export-pdf'),
                 url(r'^(?P<id>\d+)/pdf/$', plan.export_pdf,
                     name='export-pdf'), ]

# sub patterns for meals
patterns_meal = [url(r'^(?P<plan_pk>\d+)/meal/add/$',
                     login_required(meal.MealCreateView.as_view()), name='add'),
                 url(r'^(?P<pk>\d+)/edit/$',
                     login_required(meal.MealEditView.as_view()), name='edit'),
...
patterns_ingredient = [
    url(r'^(?P<pk>\d+)/delete/$', ingredient.IngredientDeleteView.as_view(),
        name='delete'),
    url(r'^(?P<pk>\d+)/edit/$', ingredient.IngredientEditView.as_view(),
        name='edit'),
    url(r'^add/$', login_required(ingredient.IngredientCreateView.as_view()),
        name='add'),
    url(r'^overview/$', ingredient.IngredientListView.as_view(), name='list'),
    url(r'^pending/$', ingredient.PendingIngredientListView.as_view(),
        name='pending'),
    ...
    url(r'^(?P<id>\d+)/view/(?P<slug>[-\w]+)/$', ingredient.view,
        name='view'), ]
...
patterns_weight_unit = [
    ...
    url(r'^(?P<pk>\d+)/delete/$', unit.WeightUnitDeleteView.as_view(),
        name='delete'),
    url(r'^(?P<pk>\d+)/edit/$', unit.WeightUnitUpdateView.as_view(),
        name='edit'), ]

# sub patterns for weight units / ingredient cross table
patterns_unit_ingredient = [url(r'^add/(?P<ingredient_pk>\d+)/$',
...
patterns_bmi = [url(r'^$', bmi.view, name='view'),
...
patterns_calories = [url(r'^$', calculator.view, name='view'),
                     url(r'^bmr$', calculator.calculate_bmr, name='bmr'),
                     url(r'^activities$', calculator.calculate_activities,
                         name='activities'),  # JS
                     ]

urlpatterns = [url(r'^', include(patterns_plan, namespace="plan")),
               url(r'^meal/', include(patterns_meal, namespace="meal")),
               url(r'^meal/item/',
                   include(patterns_meal_item, namespace="meal_item")),
               url(r'^ingredient/',
                   include(patterns_ingredient, namespace="ingredient")),
               url(r'^unit/',
                   include(patterns_weight_unit, namespace="weight_unit")),
